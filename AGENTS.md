...
- Multilingual support uses Astro's built-in i18n (`astro.config.mjs`): `es` is the default locale served directly at `/`, `en` is served at `/en/`, with no root redirect (`prefixDefaultLocale: false`). Because the default locale is unprefixed, its page files live at the root of `src/pages/` (not in an `es/` folder). Components resolve the current language with `Astro.currentLocale` and the helpers in `src/i18n/index.js` (`getCv(locale)`, `getUi(locale)`).
...
- `src/pages/index.astro` (served at `/`, the Spanish/default locale) and `src/pages/en/index.astro` mount the section components inside `src/layouts/BaseLayout.astro`, which provides `<head>`, `hreflang` alternates, the no-flash theme preload, skip-link, and the scroll-reveal script. There is no `src/pages/es/`: the default locale lives at the page root.
